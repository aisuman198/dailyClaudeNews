...
# 要約の書き方
- 1記事の要約は **約3行（80〜180 文字程度）** を目安。
- 「詳細は原文参照」「タイトルのみ」等の逃げ文句は禁止。URL は要約の上に既に表示されているためリンク誘導は不要。
- 中身が薄いタイトルでもソース名・公開日・URL のパス情報を組み合わせて、短く事実を述べる。

# 日本語訳の付与（英語記事）
タイトルが英語の場合、見出し直下に「- 訳: <日本語タイトル訳>」を追加する。日本語タイトルなら不要。
...
- 続けて「## カテゴリ別まとめ」見出しを置き、その下に「### カテゴリ名（N件）」見出しを並べる
  - 各ニュースの構成（順番厳守）:
    ...
    4. - URL: ...
    5. - 公開日: YYYY-MM-DD
    6. - 要約: 約3行、事実のみ、逃げ文句禁止
    7. 継続話題なら - 状態: 継続話題（初出: ..., 言及 N 回目）
    8. mergedFrom があれば - 関連リンク: <URL>
  - カテゴリ内ではスコア降順 → 公開日降順
