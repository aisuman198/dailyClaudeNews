...
export function buildPrompt(fresh: NewsItem[], recurring: NewsItem[]): string {
  return [
    ...
    ``,
    `# 推論の禁止（重要）`,
    `各記事の要約は、入力に含まれる事実（タイトル・URL・ソース・publishedAt・score）のみを根拠にしてください。記事本文は読めないため、本文に書かれていそうな内容を推測・補完してはいけません。「とみられる」「示唆」「期待される」「示している」等の解釈表現は禁止です。タイトルが英語の場合は原意を保った訳でとどめ、ふくらまさないでください。情報が乏しい記事は「タイトルのみ。詳細は原文参照」程度で構いません。`,
    ``,
    `# カテゴリの選び方`,
    ...
    `  - 各カテゴリの中に該当ニュースを並べる`,
    `  - 各ニュースは「#### 元タイトル」「- ソース: ...」「- URL: ...」「- 公開日: YYYY-MM-DD」「- 要約: タイトルから読み取れる事実のみを 1〜3 行で」`,
    `  - 継続話題は「- 状態: 継続話題（初出: YYYY-MM-DD, 言及 N 回目）」を追加`,
    `  - 入力に \`mergedFrom\` がある場合は「- 関連リンク: <他ソースのURL>」を追加`,
